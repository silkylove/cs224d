...
def make_windowed_data(data, start, end, window_size=1):
    ...
    for sentence, labels in data:
        ...
        start_n=[start for i in range(window_size)]
        end_n=[end for i in range(window_size)]
        n= len(sentence)
        start_n+=sentence
        start_n+=end_n
        for i in range(window_size,window_size+n):
            mixed=(sum(start_n[(i - window_size):(i + window_size + 1)],[]), labels[i-window_size])
            windowed_data.append(mixed)
    ### END YOUR CODE
