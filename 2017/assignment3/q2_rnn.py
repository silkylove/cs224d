...
def pad_sequences(data, max_length):
    ...
    mask=[zero_vector for i in range(max_length)]
    mask_label=[4 for i in range(max_length)]
    for sentence, labels in data:
        ### YOUR CODE HERE (~4-6 lines)
        sentence_mask=sentence+mask
        labels_mask=labels+mask_label
        mask_mask=[True]*len(sentence)+[False]*max_length
        ret.append((sentence_mask[:max_length],labels_mask[:max_length],mask_mask[:max_length]))
        ### END YOUR CODE ###
    return ret
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o=tf.nn.dropout(o,keep_prob=dropout_rate)
                y=tf.matmul(o,U)+b2
                preds.append(y)
                ### END YOUR CODE
