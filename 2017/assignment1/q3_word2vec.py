...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    y_=np.identity(y.shape[0])[target]
    cost=-np.log(y[target])
    gradPred=(y-y_).reshape(1,-1).dot(outputVectors).flatten()
    grad=(y-y_).reshape(-1,1).dot(predicted.reshape(1,-1))
    ### END YOUR CODE
